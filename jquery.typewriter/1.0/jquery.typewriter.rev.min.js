...
setInterval(function(){$('#typewriter-suffix').animate({opacity:0}).animate({opacity:1})},settings.blinkSpeed);var blobURL=URL.createObjectURL(new Blob(['(',function(){onmessage=function(e){self.currentStringIndex=0;self.currentCharIndex=0;self.settings=e.data;self.sInt=null;self.eInt=null;self.once=!1;start()};function start(){if(self.sInt!==null){clearInterval(sInt);if(once===!1){setTimeout(start,settings.waitingTime);once=!0;return}
once=!1;sInt=null;eInt=setInterval(function(){var data=[];data[0]=1;postMessage(data)},settings.typeDelay)}else{clearInterval(eInt);eInt=null;var currentString=settings.text[Math.floor(currentStringIndex/2)];sInt=setInterval(function(){var data=[];data[0]=0;data[1]=currentString.charAt(currentCharIndex);postMessage(data);currentCharIndex++},settings.typeDelay)}
